...
def run() -> None:
    with tempfile.TemporaryDirectory(prefix="agentrelay-watchdog-") as raw:
        ...
        miss = run_watchdog(cfg(miss_root), run_id=run_id, after_step=17, watchdog_id="wd-miss", poll_factory=lambda: SimpleNamespace(poll_once=lambda: (calls.append(1) or PollResult("idle"))), sleep=miss_clock.sleep, clock=miss_clock, poll_interval_seconds=10, max_polls=10, ui_spawn=lambda: None)
        ...
        assert timeout == "failed" and timeout_status["status"] == "FAILED" and timeout_status["finish_reason"] == "poll_owner_termination_unverified" and timeout_status["last_poll_action"] == "POLL_TIMEOUT"
        ...
        assert miss_status["poll_interval_seconds"] == 10
        print("WATCHDOG_POLL_CADENCE_10S_PASS")
