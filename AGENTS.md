...
| `response_timeout`, `response_protocol_error`, or `response_ui_error` | Reconcile the same request. Courier archives a rejected capture and reads fresh Chat state instead of parsing the same bad file forever. If the final post-request assistant turn is a Courier envelope for a different request ID, Courier first tries page-native regeneration, then may send one clearly labeled recovery turn with the same logical request ID and immutable payload. A second or uncertain attempt freezes only this request. |
...
it will search for the matching reply before any other action.

Use `courier_reconcile <request-dir>` as the mechanical recovery entry point.
It merges completed observations into `conversation-ledger.json`, prefers an
exact reply `REQUEST_ID`, and treats cursors only as hints. Two independent
ready/non-streaming observations at least 30 seconds apart may prove absence
well enough for one automatic same-payload resend; any anchor cancels it, and a
second uncertain result freezes only that request for Supervisor review.
