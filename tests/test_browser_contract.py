...
class BrowserContractTests(unittest.TestCase):
    ...
    def test_interrupted_final_reply_uses_same_native_retry(self):
        class EmptyControls:
            def count(self): return 0
            def nth(self, _index): raise AssertionError("no control")
        ...
        class Control:
            ...
            def is_visible(self): return True
            def is_enabled(self): return True
            def click(self, **_kwargs): self.page.nodes[1].text = "Thinking"
        ...
        self.assertEqual(result["method"], "page_native_regenerate")
        self.assertEqual(result["conflicting_request_ids"], [])
        self.assertEqual(len(intents), 1)
