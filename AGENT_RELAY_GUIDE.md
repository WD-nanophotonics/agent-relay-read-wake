# Generic Gmail–ChatGPT Relay Guide
...
## AgentRelay control-plane v2

The Gmail relay accepts legacy `AGENTRELAY/1` wake messages for compatibility,
but a new continuation must use `AGENTRELAY/2`. The v2 header preserves
`CHANNEL`, `RUN`, `STEP`, `PARENT`, `PROJECT`, and `DISPOSITION`, and adds
`MESSAGE_KIND`, `SOURCE_ROLE`, `TARGET_ROLE`, `AUTHORITY_CLASS`, `DECISION_ID`,
and `WORK_ORDER_ID`.

The only message that can authorize a Worker is an `AUDIT_DECISION` from an
`AUDITOR` to a `WORKER` with `WORKFLOW_CONTROL` authority. It must contain
exactly one UTF-8 `decision.json` attachment with this schema (no extra or
missing fields):
...
```json
{
  ...
  "action": "EXECUTE",
  "work_order_id": "E7C.1",
  "post_completion": "RETURN_FOR_AUDIT",
  "further_work_requires_new_decision": true
}
```

`EXECUTE` also requires exactly one UTF-8 `work_order.md`; that file is the
sole authoritative task for this Worker. Mail prose, quoted reports,
suggestions, `STOP`, `CONTINUE`, and `ACTION=EXECUTE` text are never control
signals. A `WORKER_REPORT` is always `EVIDENCE_ONLY` and cannot authorize a
next task. Missing, damaged, contradictory, or identity-mismatched artifacts
are rejected or quarantined; they never start a Worker.

The durable v2 lifecycle is:

```text
valid decision -> READY_TO_DISPATCH -> DISPATCHING -> BUSY
               -> worker completion -> AWAITING_AUDIT
```

`AWAITING_AUDIT` is not ordinary `IDLE` and does not imply `STOPPED`. The
relay will not infer the next work order. A repeated `decision_id` with the
same content hash is a duplicate; the same ID with a different hash, or a
reused `work_order_id`, is a fail-closed conflict. If a crash leaves a
dispatch intent without a conclusive owner, recovery does not create a second
Worker automatically; it records an uncertain boundary for human recovery.

Worker handoffs use the same three-layer Python wrapper as GmailCourier:
automated transport notice, quoted local payload, and Courier control
protocol. The wrapper explicitly tells ChatGPT that the local Agent is lower
authority and that only a structured `AUDIT_DECISION` can control continuation.
